...
def bgr2hsv(color: list):
    ...
    if hue <= 15:
        ...
        upper_limit = np.array([hue + aditional_bound, 255, 255], dtype=np.uint8)
    elif hue >= 165:
        lower_limit = np.array([hue - aditional_bound, 100, 100], dtype=np.uint8)
        upper_limit = np.array([180, 255, 255], dtype=np.uint8)
    else:
        # Every color other than red can be parsed normally using this strategy.
        lower_limit = np.array([hue - aditional_bound, 100, 100], dtype=np.uint8)
        upper_limit = np.array([hue + aditional_bound, 255, 255], dtype=np.uint8)
